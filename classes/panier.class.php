...
class PANIER
{
    ...
    public function addNewpanier($cnom,$pnom,$qte,$statut)
     ...
     public function viderpanier(){
        unset($_SESSION['nomproduit']); 
        unset($_SESSION['$prixunitaire']);
        unset($_SESSION['$prixtotal']);
     }

     public function getprix($pid){
         $req = 'SELECT prix FROM produit WHERE pid = :pid';
         $result = $this->cnx->prepare($req);
         $result->bindparam(':pid',$pid);
         $result->execute();
         return $result;
     }
    // public function returnprixtotal(){
    //     $_prixtotal=$_SESSION['qte'] *$_SESSION['$prixunitaire'];
    // }
}
?>